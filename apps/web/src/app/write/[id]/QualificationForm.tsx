      ...
      {fields.map((field, index) => (
        <div className="w-full flex flex-col gap-4" key={field.id}>
          <div className="flex justify-between w-full">
            <input className="text-h4 text-black placeholder:text-gray-400 w-full outline-none" placeholder="자격증명" defaultValue={field.name} {...register(`qualifications.${index}.name`, { required: `${index}번 자격증 이름` })} />
          ...
          <div className="w-full flex flex-col gap-8">
            <div className="w-full flex justify-between">
              <InputTemplate>
                ...
                <Input width={247} placeholder="점수 또는 급을 입력하세요" defaultValue={field.score} {...register(`qualifications.${index}.score`, { required: `${index}번 자격증 점수/급` })} />
              </InputTemplate>
              <InputTemplate>
                <Label accent>취득 날짜</Label>
                <Input width={512} type="date" placeholder="YYYY-MM-DD" defaultValue={field.date} {...register(`qualifications.${index}.date`, { required: `${index}번 자격증 취득 날짜` })} />
              </InputTemplate>
            </div>
          </div>
        </div>
