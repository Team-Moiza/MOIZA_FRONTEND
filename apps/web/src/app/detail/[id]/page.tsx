  ...
  return (
    <div className="relative">
      <div id="education" className="w-screen pt-[80px] px-[200px]">
        <button onClick={handleLikeClick} className="fixed bottom-10 right-60 translate-y-[-50%] size-[54px] flex items-center justify-center rounded-full shadow-[0px_2px_8px_0px_rgba(0,0,0,0.16)]">
              ...
              <Section id="links" title="링크">
                <div className="flex flex-col gap-2">
                  {rData?.links.map(({ url }, i) => (
                    <div key={i} className="w-full px-5 py-3 rounded-lg border border-gray-200 cursor-pointer">
                      {url && (
                        <span className="text-black text-p5 flex items-center gap-3 underline" onClick={() => window.open(url)}>
                          <LinkIcon size={18} />
                          {url}
                        </span>
                      )}
      ...
      {isError && (error as AxiosError).response?.status === 404 && <NoPermission />}
    </div>
  );
};
